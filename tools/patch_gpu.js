...
const startIdx = text.indexOf('async function runThreeJSTest(config, runId) {');
const endIdx = text.indexOf('// 6. WebCrypto', startIdx);

if (startIdx === -1 || endIdx === -1) {
    console.error('Could not find runThreeJSTest bounds');
    process.exit(1);
}

const newFunc = `async function runThreeJSTest(config, runId) {
            if (!supportsWebGL()) {
            ...
            return new Promise(async (resolve, reject) => {
                ...
                try {
                    ...
                    p1Frame.getStatus = () => \`幾何實體化: \${(p1Instances / 1000).toFixed(1)}k obj\`;
                    ...
                    p2Frame.getStatus = () => \`流體分形運算: \${p2Iters} 疊代\`;
                    ...
                    p3Frame.getStatus = () => \`PBR 動態陰影光源: \${p3LightCount} 盞\`;
                    ...
                    resolve({ value: unifiedScore, details: \`Phase1: \${scorePhase1}, Phase2: \${scorePhase2}, Phase3: \${scorePhase3}\`});
...
fs.writeFileSync('js/app.js', newText, 'utf-8');
console.log('Successfully patched runThreeJSTest');
