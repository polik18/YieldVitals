...
# The language codes googletrans accepts might differ slightly
lang_map = {
    ...
    'da': 'da', 'no': 'no', 'cs': 'cs', 'el': 'el', 'he': 'he', 'uk': 'uk'
}
...
for file_path in glob.glob('locales/*.json'):
    ...
    if lang in ['en', 'zh-TW', 'zh-CN']:
        continue # Already manually translated
    ...
    try:
        # bulk translate
        translations = translator.translate(texts_to_translate, dest=g_lang)
        for i, k in enumerate(keys_to_translate):
            data[k] = translations[i].text
